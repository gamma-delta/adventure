...
void introduce_object(Object* o) {
	list_of_objects.push_back(o); //insert the index of o
}
...
void display_map() {
	...
	std::cout << main_output; //show the output, no \n
	for (int y = 0; y < MAP_HEIGHT; y++) {
		for (int x = 0; x < MAP_WIDTH; x++) {
...
int main() {
	...
	for (int y = 0; y < MAP_HEIGHT; y++) {
		for (int x = 0; x < MAP_WIDTH; x++) {
			Object* floor = new Object(1, x, y);
			introduce_object(floor); //fill the map with floor
		}
	}
